    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-5">
        ...
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          className="text-xl p-2 rounded hover:bg-[#eaf3fb] hover:scale-110 transition-all text-[#0d3b66] shrink-0"
        >
          <FiMenu />
        </button>
      </div>
      ...
      <nav className="flex-1 flex flex-col gap-1 px-2">
        {sidebarLinks.map((link) => {
          ...
          return (
            ...
            >
              <span className={`text-xl shrink-0 ${isActive ? "text-white" : "text-[#27a1ff]"}`}>
                {link.icon}
              </span>
              <span 
